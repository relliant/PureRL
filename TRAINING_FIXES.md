...
针对 `2026-09-13_11-35-53_flat_baseline/model_10000.pt` 的短促弹跳问题，修正了完整接触组合、持续支撑和交替落脚判定；增加物理子步腾空惩罚；身体和脚高度改用实际地面参考。Flat 抬脚目标改为随相位变化、峰值 4 cm，并增加 `Gait/` 日志。

旧轨迹使用新规则重新计分后，接触项从每秒 **+0.172** 变为 **-0.540**，另产生每秒 **-0.039** 的持续无支撑惩罚。单脚抬起不再降低身体高度奖励。这验证了原先的计分缺口已被修正，不代表旧策略已经学会走路。

观测仍为 261 维，已有模型可以加载。新增 YAML 字段见当前 presets；旧的自定义环境 YAML 需要补齐 `commands.lin_vel_tracking_std` 和新增的 `gait` 字段。正式基线建议创建新 run，从头训练；不要把旧模型的总回报与新奖励下的回报直接比较。

规则与参数见 [步态奖励说明](GAIT_REWARD_TUNING.md)，实测数据见 [本次验证](reports/gait_fix_20260914/validation.md)。

验证：138 项测试通过；256 个环境的 100 轮真实 PPO 微调及 8 个环境的回放完成。腾空比例从约 13.5% 降到 8.3%，但有效交替落脚仍为 0，尚未得到可用步态。另修复了没有回合结束时空日志产生 NaN 的问题，并通过真实 RSL runner 回归检查。
