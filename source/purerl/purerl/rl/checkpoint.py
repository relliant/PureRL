...
def find_checkpoint(
    ...
    *,
    ...
    """Return the newest lexically sorted run/checkpoint matching both patterns."""
    ...
    if explicit_run.is_dir():
        ...
        runs = sorted(path for path in root.iterdir() if path.is_dir() and re.fullmatch(load_run, path.name))
        if not runs:
