...
原 PureRL 奖励只有「速度跟踪 + 能量惩罚」这类经典 legged 风格奖励，唯一与步态相关的 `feet_air_time` 实现也很弱（单支撑相取最小腾空时间），**没有步态相位概念**，导致训出来的策略步态异常（拖步、左右不对称、螃蟹步、原地晃等）。
...
1. **`cycle_time`**：先观察步态是否稳定交替。拖沓/慢 → 调小（0.45）；急促/不稳 → 调大（0.55–0.6）。
