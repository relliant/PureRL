...
def _check_contact_material_friction(env: TienKungLocomotionEnv, torch_module) -> None:
    ...
    low_static, high_static = env.cfg.randomization.static_friction_range
    low_dynamic, high_dynamic = env.cfg.randomization.dynamic_friction_range
    assert torch_module.all((expected[:, 0] >= low_static) & (expected[:, 0] <= high_static))
    assert torch_module.all((expected[:, 1] >= low_dynamic) & (expected[:, 1] <= high_dynamic))
    assert torch_module.all(expected[:, 1] <= expected[:, 0])
