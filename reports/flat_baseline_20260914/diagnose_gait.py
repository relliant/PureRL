#!/usr/bin/env python3
"""Measure a checkpoint's gait with the standard deterministic flat playback policy."""
...
class MeasuredEnv(TienKungLocomotionEnv):
    ...
    def _update_physics_step_sensors(self):
        ...
        if self._foot_body_indices is not None:
            ...
            self.physics_samples["contact"].append(self.contact_history.in_contact.clone())
...
def main():
    ...
    try:
        ...
        summary["setup"] = (
            "Flat-Play preset, seed 5, deterministic policy, LiDAR disabled; startup/reset randomization retained"
        )
...
def summarize(data, physics, cfg):
    ...
    valid_physics = physics["age"] * dt > 2.0
    if not valid.any():
        return {
    ...
    foot_z = data["foot_position"][..., 2] - cfg.gait.foot_height_offset
    ...
    return {
        ...
        "foot_origin_height_minus_offset_m_percentiles_5_50_95": np.percentile(
